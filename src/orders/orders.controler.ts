...
import { deleteOne, fetchOne, fetchingAll, insert, update } from "./orders.service";
...
export const getOne = async (c: Context) => {
  try {
    ...
    if(data==null) return c.json({"msg": "user not found"});
    return c.json(data, 200);
  } catch (e) {
    return c.json({ "error": e }, 500);
  }
...
export const deleteOneData = async (c: Context) => {
  try {
    ...
    const data= await fetchOne(Number(id));
    if (data === undefined) return c.json({ msg: "user not found" });
...
export const updateData= async(c: Context)=>{
  try {
    ...
    const data = await c.req.json();
    if(data_exit === undefined) return c.json({"msg":`user ${id} not found`},404)
