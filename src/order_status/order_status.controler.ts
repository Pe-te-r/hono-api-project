...
import { deleteOne, fetchOne, fetchingAll, insert, update } from "./order_status.service";
...
export const getOne = async (c: Context) => {
  try {
    ...
    if(data==null) return c.json({"msg": "order status not found"});
    return c.json(data, 200);
  } catch (e) {
    return c.json({ "error": e }, 500);
  }
...
export const deleteOneData = async (c: Context) => {
  try {
    ...
    const data= await fetchOne(Number(id));
    if (data === undefined) return c.json({ msg: "order status not found" });
...
export const updateData= async(c: Context)=>{
  try {
    ...
    const data = await c.req.json();
    if(data_exit === undefined) return c.json({"msg":`order status ${id} not found`},404)
